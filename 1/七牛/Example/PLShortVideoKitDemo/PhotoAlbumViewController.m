...
//PHAsset:包含具体的每个照片的资源信息
- (void)addAsset:(PHAsset *)asset {
    [self.selectedAssets addObject:asset];
//    PHImageRequestOptions:控制加载图片参数的一个类
    PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
    CGFloat width = ([UIScreen mainScreen].bounds.size.width / 5);
//    获得图片的尺寸大小,这里的大小是pixel,所以换算乘以[UIScreen mainScreen].scale.
    CGFloat scale = [UIScreen mainScreen].scale;
    CGSize size = CGSizeMake(width * scale, width * scale);
//    PHImageManager:管理 PHAsset 的一个类,相当于对一个具体资源更好地管理和筛选
...
//      PHFetchOptions 主要对获取到资源做一些配置和排序等,一般为 nil, 默认使用系统的.
...
//      PHFetchResult:相册资源,包括相册中图片的数量和获取
